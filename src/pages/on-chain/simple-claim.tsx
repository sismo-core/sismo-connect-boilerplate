...
import { abi as SimpleClaimABI } from "../../../abi/SimpleClaim.json";
import { transactions as SimpleClaimTransactions } from "../../../broadcast/03_SimpleClaim.s.sol/80001/run-latest.json";
...
export default function OnChainSimpleClaim() {
  ...
  const verify = async (responseBytes: string) => {
    ...
    const contractAddress = SimpleClaimTransactions.find((tx) => tx.contractName == "SimpleClaim")
      .contractAddress as `0x${string}`;

    const instance = new ethers.Contract(contractAddress, SimpleClaimABI, signer);
    try {
  ...
  return (
    <Container>
      <Title>Simple Claim on-chain</Title>
      {!isVerified ? (
        <>
          <SismoConnectButton
